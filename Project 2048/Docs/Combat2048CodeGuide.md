...
| 디버프 | 효과 |
|---|---|
| Fear | 플레이어의 이후 방어 획득량 감소 |
| Darkness | 다음 보드에 이동 불가능한 방해 블록 설치 |

공포는 `PlayerCombatController.DefenseBonus`를 낮춘다. 그래서 이후 방어 스킬을 쓸 때 `스킬 방어량 + DefenseBonus`로 계산되는 최종 방어 획득량이 줄어든다. 최종 획득 방어도는 0 아래로 내려가지 않는다.
...
| 디버프 | 임시 VFX |
|---|---|
| Fear | 붉은 화면 오버레이, "공포: 방어 획득 -N" 문구, 플레이어 초상화 짧은 펄스 |
